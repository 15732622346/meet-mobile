...
export function setupViewportFix() {
  ...
  const calculateViewportHeight = () => {
    ...
    if (isIOSSafari || isMobileChrome) {
      ...
      console.log('应用移动端vh修复', { 
        browser: isIOSSafari ? 'iOS Safari' : 'Mobile Chrome',
        innerHeight: window.innerHeight,
        innerWidth: window.innerWidth
      });
    } else {
      // 其他浏览器使用标准的vh单位
      document.documentElement.style.setProperty('--actual-vh', '100vh');
      document.documentElement.style.setProperty('--actual-vw', '100vw');
      console.log('使用标准vh单位（桌面端或其他浏览器）');
    }
  };
