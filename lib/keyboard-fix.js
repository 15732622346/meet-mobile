...
export function setupKeyboardFix() {
  ...
  if (isIOS) {
    ...
    document.addEventListener('touchmove', (e) => {
      // 只允许在聊天区域内滚动
      const target = e.target.closest('.mobile-chat-messages');
      if (!target) {
        e.preventDefault(); // 防止页面级滚动
      }
    }, { passive: false });
    ...
    // 定期修正位置
    setInterval(fixIOSScrolling, 100);
    
    // 输入框事件处理
    document.addEventListener('focusin', () => {
      setTimeout(fixIOSScrolling, 300);
    });
    
    document.addEventListener('focusout', () => {
      setTimeout(fixIOSScrolling, 100);
    });
  }
