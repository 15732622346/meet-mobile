...
interface CustomWidgetState extends BaseWidgetState {
...
export function CustomVideoConference({
  ...
  // 🎯 删除所有自定义状态管理，改用LiveKit原生机制
  // ❌ 删除：const [micRequestingUsers, setMicRequestingUsers] = React.useState<Set<string>>(new Set());
  // ❌ 删除：const [approvedMicUsers, setApprovedMicUsers] = React.useState<Set<string>>(new Set());
  // ❌ 删除：const [participantRenderStates, setParticipantRenderStates] = React.useState<Record<string, boolean>>({});
  // ❌ 删除：const [participantSpeakStates, setParticipantSpeakStates] = React.useState<Record<string, boolean>>({});
  // ❌ 删除：const [participantRoles, setParticipantRoles] = React.useState<Record<string, number>>({});
  ...
  //     let debugText = `调试更新 ${new Date().toLocaleTimeString()}\n`;
  //     debugText += `参与者总数: ${tracks.length}\n`;
  //     debugText += `userRole: ${userRole}\n`;
  //     debugText += `participantRenderStates: ${JSON.stringify(participantRenderStates)}\n`;
  //     debugText += `participantSpeakStates: ${JSON.stringify(participantSpeakStates)}\n`;
      
  //     tracks.forEach(track => {
  //       const participant = track.participant;
  //       const renderValue = participantRenderStates?.[participant.identity];
  //       const speakValue = participantSpeakStates?.[participant.identity];
  //       const isRender = renderValue === true;
  //       const canSpeak = speakValue === true;
        
  //       debugText += `${participant.name}(${participant.identity}):\n`;
  //       debugText += `  - 查找键: ${participant.identity}\n`;
  //       debugText += `  - 渲染状态原始值: ${renderValue} (类型: ${typeof renderValue})\n`;
  //       debugText += `  - 说话状态原始值: ${speakValue} (类型: ${typeof speakValue})\n`;
  //       debugText += `  - isRender计算结果: ${isRender} (${renderValue} === true)\n`;
  //       debugText += `  - canSpeak计算结果: ${canSpeak} (${speakValue} === true)\n`;
  //       debugText += `  - 最终显示: ${isRender ? 'true' : 'false'}, 说话: ${canSpeak ? 'true' : 'false'}\n`;
  //       debugText += `  - 三元运算符测试: ${isRender ? 'TRUE分支' : 'FALSE分支'}\n`;
  //     });
  ...
  //   updateDebugInfo();
  // }, [tracks, participantRenderStates, participantSpeakStates, userRole]);

  // 🎯 已删除：不再需要额外的API调用，直接使用LiveKit participant.attributes
  ...
  // 🎯 已删除：不再需要API调用获取角色信息，直接使用participant.attributes
  ...
  // 🎯 删除自动状态设置：现在使用LiveKit原生participant.attributes机制
  // ❌ 删除：自动设置渲染状态和说话权限的useEffect，改用participant.attributes
    ...
    <div className="custom-control-bar">
      ...
      <div className="control-center">
        ...
        {userId && userRole && userRole < 2 && roomInfo.name && (
          <button 
            className={`control-btn request-mic-btn ${!hasHost ? 'disabled' : ''}`}
            disabled={!hasHost}
            onClick={async (e) => {
              // 🔧 确保样式与交互逻辑一致
              if (!hasHost) {
            ...
            style={{
              // 🔧 确保样式与disabled属性一致
              pointerEvents: !hasHost ? 'none' : 'auto',
              opacity: !hasHost ? 0.5 : 1,
              cursor: !hasHost ? 'not-allowed' : 'pointer'
            }}
            title={hasHost ? "申请上麦" : "等待主持人进入后可申请上麦"}
          >
            🙋‍♂️ {hasHost ? '申请上麦' : '等待主持人'}
          </button>
        )}
  ...
  ), [roomInfo.name, widgetState.showSettings, widgetState.showHostPanel, handleLeaveRoom, userRole, userId, userName, isScreenSharing, toggleScreenShare, toggleHostPanel, hasHost]);
            ...
            <div className="sidebar-container" style={{ 
              ...
              <div className="chat-section" style={{ 
                ...
                <div className="chat-input-section" style={{
                  ...
                  <div className="chat-form-container" style={{ width: '100%' }}>
                    {/* 聊天输入框会自动显示在这里 */}
                  </div>
                </div>
              </div>
...
function MicParticipantTile({ currentUserRole, onApproveMic, userToken, setDebugInfo }: MicParticipantTileProps) {
